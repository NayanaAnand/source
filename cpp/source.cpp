...
PREPARE_LOGGING(source_i)

source_i::source_i(const char *uuid, const char *label) :
    source_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void source_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &source_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &source_i::<callback method>)
        in the constructor.
        ...
        source_i::source_i(const char *uuid, const char *label) :
            source_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &source_i::scaleChanged);
            addPropertyListener(status, this, &source_i::statusChanged);
        }

        void source_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void source_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
